...
int main(){
    ...
    int M = 20000; // We collect data for each cycle of M
    int N = MC_cycles/M;
    ...
    for(int q=0;q<N;q++){
        for(int n=0;n<M; n++){
            accepted = 0;
            Metropolis(spin_matrix, L, energy, magnetization, w, idum, accepted, neighbour);
            values[0] += energy;
        ...
        cout << MC_cycles_no[q] <<" Energy: "<<values[0]<<endl;
...
void Metropolis(int **spin_matrix, int n_spins, double &energy, double &magnetization, double *w, long &idum, int &accepted, int **neighbour){
    for(int row=0;row<n_spins;row++){
        for(int column=0;column<n_spins;column++){
...
void CreateStartingState(int **spin_matrix, double n_spins, double &energy, double &magnetization, long &idum, bool random, int **neighbour){
    ...
    for(int row=0;row<n_spins;row++){
        for(int column=0;column<n_spins;column++){
            energy -=  (double) spin_matrix[row][column]*(spin_matrix[neighbour[row][0]][column] +
                    spin_matrix[row][neighbour[column][0]]);
            magnetization += (double) spin_matrix[row][column];
        }
    }
}
